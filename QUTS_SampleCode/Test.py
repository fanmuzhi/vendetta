...
devices = client.getDeviceManager().getDevicesForService(DiagService.constants.DIAG_SERVICE_NAME)

for device in devices:
    diagService = DiagService.DiagService.Client(client.createService(DiagService.constants.DIAG_SERVICE_NAME, device))
    diagService.initializeService()

    diagService.setLoggingMask(QutsClient.readFile(f_dmc), Common.ttypes.LogMaskFormat.DMC_FORMAT)
