...
def onserviceevent(service_name, event_id, event_description):
    print(
        'on_service_event:service name: {0} occurred {1}  : {2}'.format(
            service_name, event_id, event_description
        )
    )
...
return_flags.flags = (
    Common.ttypes.QmiReturnFlags.PARSED_XML | Common.ttypes.QmiReturnFlags.PACKET_NAME
)
...
pdc_input = {'config_type': {'config_type': '0',}, 'ind_token': {'ind_token': '1',}}
