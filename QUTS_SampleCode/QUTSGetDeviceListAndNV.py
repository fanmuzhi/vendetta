...
def execTest(quts_client):
    ...
    deviceList =  devManager.getDeviceList(); ##gets the list of all the devices connected to the system
    
    print ("Device Details", " Serial Number ", deviceList[0].serialNumber, "  ", " Adb Serial Number ", deviceList[0].adbSerialNumber )

    
	##get the list of Qualcomm supported devices
    devices =  quts_client.getDeviceManager().getDevicesForService( DeviceConfigService.constants.DEVICE_CONFIG_SERVICE_NAME) 
    ...
    devConfig = DeviceConfigService.DeviceConfigService.Client(quts_client.createService(DeviceConfigService.constants.DEVICE_CONFIG_SERVICE_NAME, devices[0]))
