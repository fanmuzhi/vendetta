...
for item in listOfProtocols:
    if (-1 != item.description.find("Modem")):
        print( "Found HS-USB Modem Protocol handle ", item.description, "Handle", item.protocolHandle)
        unknownProtocol = item.protocolHandle
    print(item)

print("unknownProtocol = ", unknownProtocol)

deviceManager.overrideUnknownProtocol(unknownProtocol, Common.ttypes.ProtocolType.PROT_DUN)
