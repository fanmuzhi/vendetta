...
def onDataViewUpdated(viewName, dataViewSize, isDataViewCreationFinished):
    ...
    if isDataViewCreationFinished:  # this will not execute if script finishes before dataviewCreation is finised
...
def ProcessFiles(logSession  ):
    ...
    returnObjDiag.flags = Common.ttypes.DiagReturnFlags.PARSED_TEXT | Common.ttypes.DiagReturnFlags.PACKET_NAME | Common.ttypes.DiagReturnFlags.PACKET_ID |  Common.ttypes.DiagReturnFlags.PACKET_TYPE
    ...
    diagReturns.queries = ['."msg_type"', '."msg_length"', '."service_type"'] ## ['."Cyclic Shift of DMRS Symbols Slot 0"', '."Cyclic Shift of DMRS Symbols Slot 1"' ]
	
    diagReturns1 =  Common.ttypes.DiagReturns()
    diagReturns1.flags = returnObjDiag.flags
    ## the fields listed in queries will be displayedin the JSON query output
    diagReturns1.queries = ['."status"']
    returnObjDiag.fieldQueries = { Common.ttypes.DiagPacketType.LOG_PACKET : {'0x1545' : diagReturns, '0x1546':diagReturns1} }
    ...
    packetRange.beginIndex = 0
    packetRange.count = logSession.getDataPacketCount(dataPacketFilter.protocolHandleList[0])
    ...
    print("end:  waiting for data packets to be available in dataview. Duration: ", time.time() - start)
...
def GetDiagDataForMultipleFilters(logSession,allFilters):
    ...
    returnObjDiag = Common.ttypes.DiagReturnConfig()
    returnObjDiag.flags = Common.ttypes.DiagReturnFlags.PARSED_TEXT | Common.ttypes.DiagReturnFlags.SUMMARY_TEXT | Common.ttypes.DiagReturnFlags.RECEIVE_TIME_STRING    |  Common.ttypes.DiagReturnFlags.PACKET_NAME | Common.ttypes.DiagReturnFlags.PACKET_ID |  Common.ttypes.DiagReturnFlags.PACKET_TYPE 
    diagReturns =  Common.ttypes.DiagReturns()
    diagReturns.flags = returnObjDiag.flags
    ## the fields listed in queries will be displayedin the JSON query output
    diagReturns.queries = ['."msg_type"', '."msg_length"', '."service_type"'] ## ['."Cyclic Shift of DMRS Symbols Slot 0"', '."Cyclic Shift of DMRS Symbols Slot 1"' ]
    ##diagReturns.queries = ['."Serving Cell ID = 3"']
    returnObjDiag.fieldQueries = { Common.ttypes.DiagPacketType.LOG_PACKET : {'0x1545' : diagReturns} }
